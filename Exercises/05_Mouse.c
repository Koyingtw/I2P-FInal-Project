...
void allegro5_init(void) {
    ...
    if (!al_install_mouse()) {
        game_abort("faled to install mouse");
    }
...
void game_start_event_loop(void) {
    ...
    while (!done) {
        ...
        //    whether it is mouse move or mouse scroll.
            ...
            game_log("mouse button %d down", event.mouse.button);
            mouse_state[event.mouse.button] = true;
        }
        else if (event.type == ALLEGRO_EVENT_MOUSE_BUTTON_UP) {
            game_log("mouse button %d up", event.mouse.button);
            mouse_state[event.mouse.button] = false;
        }
        else if (event.type == ALLEGRO_EVENT_MOUSE_AXES) {
            if (event.mouse.dx != 0 || event.mouse.dy != 0) {
                game_log("mouse move from (%d %d) to (%d %d)", mouse_x, mouse_y, event.mouse.x, event.mouse.y);
...
void game_update(void) {
    ...
    else if (mouse_state[2] == true) {
        color = al_map_rgb(255, 255, 0);
    }
    else if (mouse_state[3] == true) {
